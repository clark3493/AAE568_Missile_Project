...
%
%       This version attempts to incorporate aerodynamic forces in the
%       missile dynamics.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all; clc;
...
global CL_alpha CL0 CD_alpha CD0 Aref rho;
CL_alpha = .04;
CL0 = .06;
...
function [x_,z_,xdot_,zdot_] = f(x,z,xdot,zdot,u_theta,dt)
    ...
    [Fx,Fz] = aero_forces(xdot,zdot,u_theta);
    
    x_ = x + dt .* xdot;
    z_ = z + dt .* zdot;
    xdot_ = xdot + dt .* ( Fx + T/m .* cosd(u_theta) );
    zdot_ = zdot + dt .* ( Fz - T/m .* sind(u_theta) + g );
end

function [Fx,Fz] = aero_forces(xdot,zdot,theta)
    global CL_alpha CL0 CD_alpha CD0 Aref rho;
    ...
    CD = CD_alpha .* abs(alpha) + CD0;
    
    L = 0.5 * rho * V.^2 * Aref .* CL;
    D = 0.5 * rho * V.^2 * Aref .* CD;
    
    Fx =  L.*cosd(90-eta) - D.*cosd(eta);
    Fz = -L.*sind(90-eta) - D.*sind(eta);
end
